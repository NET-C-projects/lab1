...
void PrototypeGen::SetN(std::string _N) {
  for (int i = 0; i < _N.size(); i++)
    if (!(_N[i] >= '0' || _N[i] <= '9' || _N[i] == '-')) Shortage = NotANumber;
  ...
  if (Shortage == NotANumber) throw std::invalid_argument("������� �� �����");
  if (Shortage == Exception) throw std::invalid_argument("N ������ 1");
}
